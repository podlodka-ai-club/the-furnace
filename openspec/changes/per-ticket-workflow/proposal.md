...
Concept §2 specifies one Temporal workflow per Linear ticket with phase-level activities for spec, code, and review. This change stands up that skeleton — with activities as no-ops — so agent implementations in later changes plug directly into a durable, signal-driven shell.
...
- Add workflow signals: `cancel`, `approveMergeVeto` (placeholder for later use by `vote-aggregator`).
...
- The three phase activities are explicit extension points: later changes (`spec-agent`, `coder-agent`, `persona-reviewers` + `vote-aggregator`) replace the no-ops with real logic.
