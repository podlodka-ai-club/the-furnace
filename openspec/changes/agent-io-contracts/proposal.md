...
Concept §2 pipeline passes artefacts between phases — Linear ticket -> failing tests -> green tests + diff -> review verdict -> PR open — but the shape of each artefact is currently fixed only in prose scattered across individual proposals (`spec-agent`: *"branch + test-commit manifest"*, `coder-agent`: *"diff manifest"*, `review-agent`: inline verdict payload). Without a single schema each phase interprets its neighbour's shape on its own: the spec agent returns one test manifest, the coder expects another, and review output drifts as prompts evolve.
...
- Create `server/src/agents/contracts/` — a single module for inter-agent schemas, one file per pipeline boundary:
  ...
  - `reviewer-io.ts` — `ReviewerInput` (green tests + diff + ticket metadata) and `ReviewResult = { verdict: "approve"|"changes_requested", reasoning, findings: string[] }`.
  - `shared.ts` — common types (`SubTicketRef`, `DiffStat`, `TestRunSummary`).
...
- `ticket-workflow` (from `per-ticket-workflow`): signatures of the phase activities (`runSpecPhase`, `runCoderPhase`, `runReviewPhase`) become typed as `SpecPhaseOutput | CoderPhaseOutput | ReviewResult` instead of `any`/`void`. The no-op implementations return placeholder values that are valid against the corresponding schemas.

## Impact

- New dep: `zod` (~50kb, zero peer deps, serialises cleanly through the Temporal JSON payload converter).
- New files: `server/src/agents/contracts/{spec-output,coder-output,reviewer-io,shared,index}.ts`, plus `server/tests/agents/contracts/*.test.ts`.
...
- Depended on by (each adds a line to its own `Depends on` during implementation): `per-ticket-workflow`, `spec-agent`, `coder-agent`, `review-agent`.
- Advanced governance contracts for `persona-reviewers` and `vote-aggregator` are intentionally deferred with those changes to Phase 6.
