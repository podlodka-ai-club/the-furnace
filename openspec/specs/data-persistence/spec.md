...
Provides the PGLite-backed persistence layer that underpins the orchestration runtime: a typed database handle created by a pure factory, a forward-only migration runner that tracks applied versions, an initial schema defining the core orchestration tables (`workflow_runs`, `tickets`, `attempts`, `reviews`, `provenance`), TypeScript row interfaces kept in sync with that schema, Express wiring that threads the handle through `app.locals`, and a dev data directory at `data/pglite/` so local work persists across restarts without requiring a running Postgres.
