...
The spec agent SHALL be given exactly two custom tools — `propose_failing_tests` and `request_ac_clarification` — and MUST commit to one of them as its terminal action. Free-form prose without a tool call SHALL be treated as a model failure.

#### Scenario: Agent proposes failing tests

- **WHEN** the agent calls `propose_failing_tests({ files: [{ path, contents, description }, …] })`
- **THEN** the activity MUST treat the call as the agent's terminal decision
- **AND** the activity MUST proceed to write each file, verify failure, and commit each file
